...
class MainWindow(QMainWindow):
    ...
    def toolbar(self):
        ...
        notes = QAction(QIcon("Icons/Note.png"), "Notes", self)
        notes.triggered.connect(self.notes_panel)
        ...
        navigation_bar.addSeparator()
        navigation_bar.addActions([notes])
    ...
    def instant_search_dialog(self):
        try:
            ...
            self.input_dialog.setStyle(QStyleFactory.create("fusion"))
            self.input_dialog.setFixedWidth(500)
            ...
            wikipedia = QPushButton()
            wikipedia.setStyle(QStyleFactory.create("fusion"))
            wikipedia.setText("Wikipedia")
            wikipedia.clicked.connect(self.wiki_search)

            britainica = QPushButton()
            britainica.setStyle(QStyleFactory.create("fusion"))
            britainica.setText("Britannica")
            britainica.clicked.connect(self.britannica_search)

            load_local = QPushButton()
            load_local.setStyle(QStyleFactory.create("fusion"))
            load_local.setText("Load Local File")
            load_local.clicked.connect(self.local_search_tab_checker)

            load_as_pdf = QPushButton()
            load_as_pdf.setStyle(QStyleFactory.create("fusion"))
            load_as_pdf.setText("Load as PDF")
            load_as_pdf.clicked.connect(self.pdf_path_checker)

            load_as_img = QPushButton()
            load_as_img.setStyle(QStyleFactory.create("fusion"))
            load_as_img.setText("Load as an Image")
            load_as_img.clicked.connect(self.image_viewer_checker)

            toolbar.addWidget(spacer)
            toolbar.addWidget(wikipedia)
            toolbar.addWidget(britainica)
            toolbar.addWidget(load_local)
            toolbar.addWidget(load_as_pdf)
            toolbar.addWidget(load_as_img)
    ...
    def pdf_path_checker(self):
        if self.input_dialog.text() == "":
            path, _ = QFileDialog.getOpenFileName(self, name, "", "PDF Files (*.pdf)")

            if path and _:
                self.pdf_view(os.path.abspath(path))
        else:
            self.pdf_view(self.input_dialog.text())

        self.dialog.destroy()

    def image_viewer_checker(self):
        if self.input_dialog.text() == "":
            path, _ = QFileDialog.getOpenFileName(self, name, "", "PNG Files (*.png);; JPG Files (*.jpg, *.jpeg, *.jfif);;")

            if path and _:
                self.image_viewer(os.path.abspath(path))
        else:
            self.image_viewer(self.input_dialog.text())

    def local_search_tab_checker(self):
        if self.input_dialog.text() == "":
            path, _ = QFileDialog.getOpenFileName(self, name, "", "All Files (*.*);;")

            if path and _:
                self.local_search_tab(os.path.abspath(path))
        else:
            self.local_search_tab(self.input_dialog.text())
    ...
    def local_search_tab(self, text):
        query = text
        page = WebEnginePage(self.main_profile, self)
    ...
    def download_manager(self):
        ...
        self.download_list_widget.setStyle(QStyleFactory.create("fusion"))
        ...
        clear_download_history = QPushButton()
        clear_download_history.setText("Clear")
        clear_download_history.setStyle(QStyleFactory.create("fusion"))
        clear_download_history.clicked.connect(self.clear_download_history)

        exit_function = QPushButton()
        exit_function.setText("Close")
        exit_function.setStyle(QStyleFactory.create("fusion"))
        exit_function.clicked.connect(lambda: manager.destroy())

        toolbar.addWidget(spacer)
        toolbar.addWidget(clear_download_history)
        toolbar.addWidget(exit_function)
    ...
    def number_guesser(self):
        ...
        self.guess_bar.setStyle(QStyleFactory.create("fusion"))
        ...
        label2.setFont(font_2)

        layout.addWidget(label2)

        self.result = QListWidget()
        self.result.setStyle(QStyleFactory.create("fusion"))
    ...
    def shortcuts(self):
        ...
        print_to_pdf = QShortcut(QKeySequence("Ctrl+P"), self)
        print_to_pdf.activated.connect(self.print_page_to_pdf)

        normal = QShortcut(QKeySequence("Esc"), self)
        normal.activated.connect(lambda: self.showMaximized())
    ...
    def notes_panel(self):
        self.text_edit_dialog = QDialog()
        self.text_edit_dialog.setWindowTitle("Notes")
        self.text_edit_dialog.setFixedWidth(1000)
        self.text_edit_dialog.setFixedHeight(600)
        ...
        new_doc = QPushButton()
        new_doc.clicked.connect(self.new_doc)
        new_doc.setStyle(QStyleFactory.create("fusion"))
        new_doc.setText("New")
        toolbar.addWidget(new_doc)

        open_doc = QPushButton()
        open_doc.clicked.connect(self.open_doc)
        open_doc.setStyle(QStyleFactory.create("fusion"))
        open_doc.setText("Open")
        toolbar.addWidget(open_doc)

        save = QPushButton()
        save.setStyle(QStyleFactory.create("fusion"))
        save.clicked.connect(self.save)
        save.setText("Save")
        toolbar.addWidget(save)

        save_as = QPushButton()
        save_as.setStyle(QStyleFactory.create("fusion"))
        save_as.clicked.connect(self.save_doc)
        ...
        self.font_box.setStyle(QStyleFactory.create("fusion"))
        self.font_box.setCurrentFont(QFont("Calibri"))
        self.font_box.currentFontChanged.connect(lambda: self.text_edit.setCurrentFont(self.font_box.currentFont()))
        toolbar.addWidget(self.font_box)

        self.font_box_size = QSpinBox()
        self.font_box_size.setStyle(QStyleFactory.create("fusion"))
        self.font_box_size.setMaximum(100)
        self.font_box_size.setMinimum(8)
        self.font_box_size.setValue(12)
        self.font_box_size.valueChanged.connect(lambda: self.text_edit.setFontPointSize(self.font_box_size.value()))
        toolbar.addWidget(self.font_box_size)
        ...
        self.text_edit.setStyle(QStyleFactory.create("fusion"))
        self.text_edit.setFont(QFont("Calibri", 12))
        self.text_edit.document().setDocumentMargin(72)
        self.text_edit.textChanged.connect(lambda: self.update_word_count(self.text_edit.toPlainText()))
        layout.addWidget(self.text_edit)

        self.status_bar = QLabel()
        self.status_bar.setText(f"Word Count: {str(len(self.text_edit.toPlainText().split()))}")
        layout.addWidget(self.status_bar)

        self.status_bar_2 = QLabel()
        self.status_bar_2.setText(f"Character Count: {str(len(self.text_edit.toPlainText()))}")
        layout.addWidget(self.status_bar_2)


        self.text_edit_dialog.setLayout(layout)
    ...
    def open_doc(self):
        ...
        if path and _:
            if path.endswith(".pdf"):
                ...
                self.image_viewer(path)
                self.text_edit_dialog.destroy()
            elif path.endswith(".jfif"):
                self.image_viewer(path)
                self.text_edit_dialog.destroy()
            else:
                self.path = path

                if path.endswith(".ptxt"):
                    with open(path, "r", encoding="utf-8") as f:
                        self.text_edit.setText(f.read())
                        self.text_edit_dialog.setWindowTitle(f"Viewing {os.path.basename(path)}")
                        f.close()
                else:
                    with open(path, "r", encoding="utf-8") as f:
    ...
    def save_doc(self):
        path, _ = QFileDialog.getSaveFileName(self, name, "", "All Files (*.*)")
        ...
        if path and _:
            if path.endswith(".pdf"):
                ...
                QMessageBox.information(self, name, f"Has been exported as {os.path.basename(path)}")
                self.text_edit_dialog.setWindowTitle(f"Viewing {os.path.basename(path)}")
            elif path.endswith(".ptxt"):
                self.path = path
                ...
                QMessageBox.information(self, name, f"Has been exported as {os.path.basename(self.path)}")
                self.text_edit_dialog.setWindowTitle(f"Viewing {os.path.basename(self.path)}")

    def save(self):
        if self.path == None:
            self.save_doc()
        else:
            if self.path.endswith(".ptxt"):
    ...
    def print_page_to_pdf(self):
        ...
        if isinstance(browser, QWebEngineView):
            path, _ = QFileDialog.getSaveFileName(self, name, "", "PDF Files (*.pdf);;")

            if path and _:
                browser.page().printToPdf(path)
                browser.page().pdfPrintingFinished.connect(lambda: self.finished_pdf(path))

    def finished_pdf(self, get_path):
        QMessageBox.information(self, name, f"Exported as {os.path.basename(get_path)}")

    def update_word_count(self, words):
        count = str(len(words.split()))
        self.status_bar.setText(f"Word Count: {count}")

        char_count = str(len(words))
        self.status_bar_2.setText(f"Character Count: {char_count}")
    ...
    def update_application(self):
        ...
        if current_day >= cutoff:
            ...
            title_text = QLabel()
            title_text.setText("An update is available, would you like to switch now?")
            ...
            remind_later = QPushButton("Remind Later", self, clicked=lambda: self.update_dialog_box.destroy())
            remind_later.setStyle(QStyleFactory.create("fusion"))

            toolbox.addWidget(update_now)
            toolbox.addWidget(remind_later)
